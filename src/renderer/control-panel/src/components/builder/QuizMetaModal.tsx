              ...
              <div className="flex-1 space-y-3">
                {[
                  ...
                  <div key={id} className="flex items-center gap-3">
                    <Label htmlFor={id} className="w-20 text-right text-sm shrink-0">{label}</Label>
                    <Input id={id} placeholder={placeholder} value={value} onChange={(e) => onChange(e.target.value)} />
                  </div>
                ))}
                <div className="flex items-center gap-3">
                  <Label htmlFor="quiz-date" className="w-20 text-right text-sm shrink-0">{t('builder.date')}</Label>
                  <Input id="quiz-date" type="date" value={meta.data.date} onChange={(e) => updateDate.mutate(e.target.value)} />
                </div>
                <div className="flex items-center gap-3">
                  <Label htmlFor="quiz-timer" className="w-20 text-right text-sm shrink-0">{t('builder.timer')}</Label>
                  <Input
                    id="quiz-timer"
                    type="number"
                    min={0}
